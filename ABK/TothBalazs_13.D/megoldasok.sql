CREATE DATABASE osztaly;

CREATE TABLE adatok (
    tanulokod INT PRIMARY KEY AUTO_INCREMENT,
    nev VARCHAR(100) NOT NULL,
    mat VARCHAR NOT NULL,
    angol VARCHAR(100) NOT NULL,
    nyelv2 VARCHAR(100) NOT NULL,
    tesi VARCHAR(100) NOT NULL,
    csaladszam INT NOT NULL,
    tesoszam INT NOT NULL

);

INSERT INTO adatok (tanulokod, nev, mat, angol, nyelv2, tesi, csaladszam, tesoszam) VALUES
...
select nev from adatok where csaladszam>1;


SELECT COUNT(*) 
FROM adatok 
WHERE (csaladszam - tesoszam) <> 3;
...
SELECT nev 
FROM adatok 
WHERE angol = (SELECT angol FROM adatok WHERE nev = 'Hát Izsák')
AND nev <> 'Hát Izsák';

SELECT * FROM adatok 
WHERE nev LIKE '%Bekre%';
